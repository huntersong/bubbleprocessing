function bubble_data = my_postprocessing(bubble,varargin)
...
for n=1:num2
    if(abs(bubble{n,2})==0.5 & bubble{n,6}~=0)
        ...
        d_bubble(num_bubble,1)=1;
        d_bubble(num_bubble,2)=2*bubble{n,6}(1,1);

    elseif(abs(bubble{n,2})==0 & bubble{n,6}~=0)
        num_bubble=num_bubble+1;
        d_bubble(num_bubble,2)=((2*abs(bubble{n,6}(1,1)))^2*2*abs(bubble{n,6}(1,2)))^(1/3);
    ...
    elseif(abs(bubble{n,2})==2)
        if(real(bubble{n,6}{1,1}(1,1))~=0 && real(bubble{n,6}{1,1}(1,2))~=0)
            num_bubble=num_bubble+1;
            d_bubble(num_bubble,2)=((2*abs(bubble{n,6}{1,1}(1,1)))^2*2*abs(bubble{n,6}{1,1}(1,2)))^(1/3);
        end
        if(real(bubble{n,6}{2,1}(1,1))~=0 && real(bubble{n,6}{2,1}(1,2))~=0)
            num_bubble=num_bubble+1;
            d_bubble(num_bubble,2)=((2*abs(bubble{n,6}{2,1}(1,1)))^2*2*abs(bubble{n,6}{2,1}(1,2)))^(1/3);
        end
    elseif(abs(bubble{n,2})>=2)
        [b,~]=size(bubble{n,6});
        for i=1:b
            if(isempty(bubble{n,6}{i,1})~=1 & bubble{n,6}{i,1}~=0)
                num_bubble=num_bubble+1;
                d_bubble(num_bubble,2)=((2*abs(bubble{n,6}{i,1}(1,1)))^2*2*abs(bubble{n,6}{i,1}(1,2)))^(1/3);
            end
        end
    end
end
d_bubble(2,3)=num_bubble;
dia_3 = 0;
dia_2 = 0;   %计算D[3，2]平均直径
for i = 1 : num_bubble
    if d_bubble(i,2)<100 & d_bubble(i,2)>2
        dia_3 = dia_3 + d_bubble(i,2)^3;
        dia_2 = dia_2 + d_bubble(i,2)^2;
    end
end
d_bubble(2,4) = dia_3/dia_2;
