...
filename = ['D:\办公电脑文件\reynoldsstress\Motion Detection\CEJ\case4\imcrop',sprintf('%02d',numb),'.bmp'];
%参数定义
%图像坐标(x,y)和像素坐标(u,v)，世界坐标(U,V,Z)，_world坐标系，_camera坐标系  _info全信息
image_camera1 = imread(filename); %假定每个图像中气泡占用的像素小于128px；
%image_camera1 = imrotate(image_camera1,-10);
...
%        P_c_camera1and2 = [238,147,-1130]; P_d_camera1and2 = [25,147,-1050];
    ...
    for m = 1:length(point1_img_camera1)    % 这是第m层的气泡切片
...
filename = ['D:\办公电脑文件\reynoldsstress\Motion Detection\CEJ\case4\',sprintf('%02d',numb),'data.mat'];
...
filename = ['D:\办公电脑文件\reynoldsstress\Motion Detection\CEJ\case4\',sprintf('%02d',numb),'datatempab.mat'];
save(filename,'tempab');
clear
end
