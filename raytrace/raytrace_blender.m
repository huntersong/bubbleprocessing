...
image_camera1 = imread("D:\办公电脑文件\reynoldsstress\Motion Detection\blender_case\blender_calibration_2\camera1.bmp"); %假定每个图像中气泡占用的像素小于128px；
...
image_camera1 = imcrop(image_camera1,[800 400 320 280]);

image_camera2 = imread("D:\办公电脑文件\reynoldsstress\Motion Detection\blender_case\blender_calibration_2\camera2.bmp"); %假定每个图像中气泡占用的像素小于128px；
if numel(size(image_camera2))>2
    fprintf('%s','彩色度图像');
    image_camera2 = rgb2gray(image_camera2);
end
image_camera2 = im2double(image_camera2);
[y,x] = size(image_camera2);
image_camera2 = imcrop(image_camera2,[800 400 320 280]);
...
%（2）{
    ...
    [point1_img_camera2,point2_img_camera2] = slice_position(imfill_img_camera2); 
            %判断两个相机的气泡高度是否一致 按照相机1的图片修正补齐
            if length(point1_img_camera1)~=length(point1_img_camera2)
                disparity = abs(length(point1_img_camera1)-length(point1_img_camera2));
                disparity_up = round(disparity/2);      disparity_down = disparity-disparity_up;
                disparity_add_p1 = point1_img_camera2(length(point1_img_camera2),1:3);
                repmatp1_up = repmat(disparity_add_p1,disparity_up,1);  repmatp1_down = repmat(disparity_add_p1,disparity_down,1);  
                point1_img_camera2 = [repmatp1_up;point1_img_camera2;repmatp1_down];
                disparity_add_p2 = point2_img_camera2(length(point2_img_camera2),1:3);
                repmatp2_up = repmat(disparity_add_p2,disparity_up,1);  repmatp2_down = repmat(disparity_add_p2,disparity_down,1);     
                point2_img_camera2 = [repmatp2_up;point2_img_camera2;repmatp2_down];
            end
...
pcshow(ptCloud);
    ...
    s = surf(bubble_position_X,bubble_position_Z,bubble_position_Y);
    colorbar
    colormap(gray)
    %sc = surfc(bubble_position_X,bubble_position_Y,bubble_position_Z,'FaceColor','b');
    hold on
    scatter3(bubble_back_position_face_world(:,1),bubble_back_position_face_world(:,3), ...
        bubble_back_position_face_world(:,2),'k.')
    ...
    numb =1;
    filename = ['D:\办公电脑文件\reynoldsstress\Motion Detection\blender_case\blender_calibtration_1\',sprintf('%02d',numb),'bubble'];
